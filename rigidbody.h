...
public:
	...
	inline uint Size() const {return mArrayAtomProp.size();};  
	...
	Coord3D GetCoords(uint i) const {return mCoords[i];}; 
	/// returns the coordinates of atom i 
	
	Coord3D GetCoordsFromType(string); 
	/// returns the coordinate of an atom with type t 
