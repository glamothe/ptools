...
if (x >= roff)
  ...
  dbl rofr = roff-x ;
  dbl rron = x-ron ;
  result[0] = rofr*rofr/pow(roff-ron,3)*(rofr+3*rron) ; //f(x)
  result[1] = -6.0*rofr*rron/(roff-ron) ; //f'(x)
  }
...
dbl ScorpionForceField::nonbon_forces(AttractRigidbody& rec, AttractRigidbody& lig, AttractPairList & pairlist, std::vector<Coord3D>& forcerec, std::vector<Coord3D>& forcelig, bool print)
{
    ...
    for (uint iter=0; iter<pairlist.Size(); iter++)
    {
        ...
        dbl vlj = elr6*vswi[0]-eij*exp(-r2s2) ;

        sumLJ += vlj;

        dbl fb = elr6*(6.0*vswi[0]-r1*vswi[1])-2.0*eij*r2s2*exp(-r2s2) ; 
